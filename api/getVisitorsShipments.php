<?php
    ...
    try {
        ...
        $shipment_stmt = $conn->prepare("SELECT `origen`, `hbl`, `ci`, `manifest`, `route_id`, `status` 
                                        FROM `shipments` 
                                        WHERE `origen` = ?");
        $shipment_stmt->bind_param("s", $user_origen);
        ...
        $header_data_clr = [
            ...
            'route_id' => 'route_id',
            'status'   => ['Detenido', 'Almacen', 'Borrador', 'Entregando', 'Entregado'],
        ];
