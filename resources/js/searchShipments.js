/* ================================================================
 *  MÓDULO: Buscador de Shipments (Envíos)
 *  Archivo: shipments-search.js
 * ================================================================ */
(function () {
    'use strict';
    ...
    const state = {
        ...
        sortBy: null,
        sortDir: 'asc',
        inputId: 'searchShipmentInput',
        resultsId: 'shipmentsResults',
        endpoint: '../api/searchShipmentsInfo.php',
        label: 'Buscar envíos',
        placeholder: 'Buscar por HBL, nombre, agencia...',
    };
    ...
    function renderScaffold() {
        ...
        root.innerHTML = `
            <div class="search-bar">
                ...
                <button type="button" class="btn btn-secondary" id="clear-shipments">
                    Limpiar
                </button>
            </div>
            <div id="${state.resultsId}">
                <div class="loading">Escriba al menos ${MIN_CHARS} caracteres para buscar envíos</div>
            </div>
        `;
        ...
        inputEl.addEventListener('input', debounce(function () {
            window.ShipmentsSearch.search(inputEl.value);
        }, 300));

        document.getElementById('clear-shipments')
    ...
    function clearSearch() {
        ...
        renderMessage(state.resultsId, 'loading',
            'Escriba al menos ' + MIN_CHARS + ' caracteres para buscar envíos');
    }
    ...
    function renderTable() {
        ...
        container.innerHTML = `
            <div class="toolbar">
                <div class="count">
                    Mostrando ${sorted.length} de ${state.rows.length} envíos
                </div>
                <div></div>
            </div>
    ...
    async function search(query) {
        ...
        renderMessage(state.resultsId, 'loading', 'Buscando envíos...');
        ...
        try {
            ...
            if (rows.length === 0) {
                renderMessage(state.resultsId, 'empty', 'No hay envíos coincidentes');
                return;
            }
            renderTable();
            ...
            renderMessage(state.resultsId, 'error',
                'Error al cargar los envíos: ' + err.message);
        }
    }
