...
function getFilteredRows() {
  const { origen, city, state: st, search } = state.filters;
  const q = search.trim().toLowerCase();

  return state.allRows.filter(row => {
    if (origen && row.origen !== origen) return false;
...
function render(data) {
    ...
    <div class="filters">
      ...
      <div class="filter-group">
        ...
        <select id="f-origen">
          ...
          ${header_data.origen.map(v => `<option value="${esc(v)}">${esc(v)}</option>`).join('')}
        </select>
      </div>
      <div class="filter-group">
  ...
  document.getElementById('f-origen').addEventListener('change', e => {
    state.filters.origen = e.target.value;
    state.page = 1;
    renderTable();
  });
  ...
  document.getElementById('clear-filters').addEventListener('click', () => {
    state.filters = { origen: '', city: '', state: '', search: '' };
    state.page = 1;
